...
var TileGridCopy = Terrain_Grid;

for (var x = 1; x < Size_x-1; x++) {
  for (var y = 1; y < Size_y-1; y++) {
    var TileCount = GetTileCount(x, y, 1);
    if (TileCount >= 3)
      Terrain_Grid[x][y] = 1;
    else
      Terrain_Grid[x][y] = 0;
...
function GetTileCount(x, y, id) {
  var Count = 0;
  if (TileGridCopy[x + 1][y + 1] == id) {
    Count++;
  }
  if (TileGridCopy[x][y + 1] == id) {
    Count++;
  }
  if (TileGridCopy[x - 1][y + 1] == id) {
    Count++;
